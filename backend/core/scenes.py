...
async def _detail_prompt(base: str, line: str, use_llm: bool) -> str:
    if not use_llm or not settings.openrouter_api_key:
        return _detail_prompt_fallback(base, line)
    try:
        user = f"General concept: {base}\nLyric line: {line or '(instrumental)'}"
        return (await llm_client.chat(get_prompts()["detail_system"], user, temperature=0.8)).strip()
...
async def plan_uniform(req: JobRequest, duration: float) -> List[Scene]:
    n = req.n_scenes if req.n_scenes > 0 else _auto_n_scenes(duration)
    step = duration / n
    scenes: list[Scene] = []
    for i in range(n):
        prompt = await _detail_prompt(req.prompt, "", use_llm=req.llm_detail)
...
async def plan_whisper(req: JobRequest, audio_path: Path, duration: float) -> List[Scene]:
    ...
    scenes: list[Scene] = []
    for i, ln in enumerate(lines):
        prompt = await _detail_prompt(req.prompt, ln.text, use_llm=req.llm_detail)
...
async def plan_llm(req: JobRequest, duration: float) -> List[Scene]:
...
async def plan_scenes(req: JobRequest, audio_path: Path, duration: float) -> List[Scene]:
    ...
    if req.scene_mode == SceneMode.llm:
        return await plan_llm(req, duration)
    return await plan_whisper(req, audio_path, duration)
